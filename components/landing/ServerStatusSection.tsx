    ...
    return (
        <section className="w-full px-4 py-16 flex flex-col items-center space-y-8">
            ...
            <div className="w-full max-w-4xl">
                ...
                {/* All Channels */}
                <div className="mb-6">
                    <h2 className="text-2xl font-bold text-red-100/90 mb-4">Channels</h2>
                            ...
                            return (
                                <div key={chNum} className="bg-stone-900/80 rounded-lg p-4 border border-red-500/20">
                                    <div className="flex flex-col items-center space-y-2">
                                        <span className="text-sm font-semibold text-red-100/70">Channel {chNum}</span>
                ...
                {status.bonuses && (
                    <div className="mb-6">
                        ...
                        {Object.values(status.bonuses).every((v: unknown) => !v || v === 0) && (
                            <div className="text-center text-red-100/60 mt-4">No active bonuses at this time</div>
                        )}
                    </div>
                )}
                {!status.bonuses && (
                    <div className="mb-6">
                        <h2 className="text-2xl font-bold text-red-100/90 mb-4">Active Bonuses</h2>
                        <div className="text-center text-red-100/60">Bonus information unavailable</div>
                    </div>
                )}
                ...
                {status.channelBonuses && status.channelBonuses.length > 0 && (
                    <div>
                        <h2 className="text-2xl font-bold text-red-100/90 mb-4">Channel Bonuses</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {status.channelBonuses.map((bonus) => {
                                ...
                                return (
                                    <div key={bonus.channelId} className="bg-gradient-to-br from-red-500/20 to-red-600/20 rounded-lg p-4 border border-red-500/30">
                                        <div className="text-sm text-red-300 font-semibold mb-3">Channel {bonus.channelId}</div>
